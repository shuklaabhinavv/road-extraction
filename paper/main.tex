...
% C2I6-2026 uses double-blind review: keep \anontrue for the submission PDF;
% change the line below to \anonfalse for the camera-ready after acceptance.
\newif\ifanon
\anontrue
...
\IEEEauthorblockA{Submitted to C2I6-2026 for double-blind review}
}
\else
\author{
\IEEEauthorblockN{Abhinav Shukla}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\
\textit{Amity School of Engineering and Technology}\\
\textit{Amity University Uttar Pradesh, Lucknow Campus, India}\\
abhinav24shukla08@gmail.com}
\and
\IEEEauthorblockN{Dr. Sachin Kumar}
\IEEEauthorblockA{\textit{Dept. of Computer Science and Engineering}\\
\textit{Amity School of Engineering and Technology}\\
\textit{Amity University Uttar Pradesh, Lucknow Campus, India}\\
skumar3@lko.amity.edu}
}
\fi
...
Road networks underpin navigation, logistics, emergency response, and urban
planning, yet the geographic databases that describe them age quickly: new
construction, informal settlements, and disasters reshape road layouts faster
than surveying and manual digitization can record them. Keeping road maps
current at scale has therefore become a bottleneck for a wide range of
location-dependent services.

Satellite and aerial imagery offer a natural remedy, providing frequent,
wide-area observations from which road geometry can in principle be recovered
\cite{cheng2016survey,zhu2017deep}. In practice, however, converting pixels
into map-ready road data is difficult: manual digitization of imagery is
labor-intensive and slow, while roads themselves are thin, kilometer-long
structures that are easily confused with rooftops, rail corridors, and field
boundaries, and are frequently interrupted by shadow and tree cover.
...
\midrule
Ensemble ($\times$4) & 0.656 & 0.792 & 0.814 & 0.772 & 94.7 & 488.3 \\
\bottomrule
\end{tabular}
\end{table}

U-Net++ attains the best single-model accuracy on every metric, supporting the
hypothesis that its nested skip pathways reduce the encoder--decoder semantic
gap and benefit thin-structure localization---but at nearly double the
inference latency of the alternatives. Averaging the probability maps of all
four models yields only a further $+0.002$ IoU at $4\times$ the cost,
indicating that under an identical training protocol the architectures make
largely \emph{correlated} errors: what one decoder misses, the others tend to
miss as well. LinkNet is the clear efficiency choice: within
...
Fig.~\ref{fig:failures} shows the two lowest-IoU test images for the best
model, with errors decomposed into hits, missed roads, and false detections.
The failure modes are systematic rather than random: the worst case
(IoU $0.402$) is a forested region whose ground truth includes roads running
\emph{under} tree canopy---invisible in the imagery and arguably unknowable
from pixels alone---while the second (IoU $0.437$) is a dense historic town
core whose narrow, tightly packed streets fall below the effective resolution
of the model. Both cases inflate missed-road counts far more than false
detections, consistent with the precision-over-recall behavior observed in
Table~\ref{tab:comparison}.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/failure_cases_unetpp.png}
\caption{Worst-case test images for U-Net++. Right column: green = correctly
detected road, red = missed road, blue = false detection. Failures concentrate
in canopy-occluded roads (top) and very dense historic street grids (bottom).}
\label{fig:failures}
\end{figure}
...
\subsection{Zero-Shot Generalization to Unseen Regions}
To probe geographic transfer, all four models were evaluated \emph{without any
retraining} on 362 tiles from the DeepGlobe road extraction dataset
\cite{demir2018deepglobe}, whose imagery covers India, Indonesia, and Thailand.
DeepGlobe's 0.5\,m/px images were downsampled $2\times$ to match the 1\,m/px
training resolution. Table~\ref{tab:deepglobe} reports the results.

\begin{table}[t]
\caption{Zero-Shot Transfer to DeepGlobe (362 tiles, no retraining).}
\label{tab:deepglobe}
\centering
\footnotesize
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Model & IoU & $F_1$ & Prec. & Rec. \\
\midrule
LinkNet      & \textbf{0.159} & \textbf{0.275} & 0.782 & \textbf{0.167} \\
DeepLabV3+   & 0.134 & 0.236 & 0.559 & 0.150 \\
U-Net++      & 0.125 & 0.222 & 0.829 & 0.128 \\
U-Net        & 0.117 & 0.210 & 0.828 & 0.120 \\
\midrule
Ensemble ($\times$4) & 0.120 & 0.214 & \textbf{0.858} & 0.123 \\
\bottomrule
\end{tabular}
\end{table}

Two findings stand out. First, absolute performance drops sharply
(IoU $0.65 \rightarrow 0.12$--$0.16$): the domain gap---different sensor,
geography, and above all road \emph{types}, with unpaved rural roads dominating
DeepGlobe---makes zero-shot transcontinental mapping impractical, motivating
region-specific fine-tuning. Second, and more interesting, \textbf{the
architecture ranking inverts under domain shift}: the in-domain winner
(U-Net++) transfers worst, while LinkNet---third at home---transfers best,
followed by DeepLabV3+. Under an identical training protocol this pattern
suggests that higher-capacity decoders fit domain-specific appearance cues
more tightly, whereas lighter decoders and atrous context aggregation retain
more transferable representations. Notably, precision remains high
($0.56$--$0.86$) while recall collapses: on unfamiliar imagery the models
rarely hallucinate roads, they simply detect only the subset that resembles
their training distribution (Fig.~\ref{fig:deepglobe}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/deepglobe_zeroshot.png}
\caption{Zero-shot predictions on DeepGlobe imagery (India/Indonesia/Thailand).
Paved, Massachusetts-like roads are detected; unpaved rural roads are largely
missed---high precision, low recall.}
\label{fig:deepglobe}
\end{figure}

\subsection{Limitations}
Three limitations qualify these results. First, each architecture was trained
once with a fixed seed; given the narrow accuracy band, run-to-run variance
could shift the single-model ranking, and the comparison is therefore best
read as ``all four are competitive, with a consistent efficiency ordering''
rather than as a strict hierarchy. Second, pixel-wise metrics do not capture
topological quality: a one-pixel gap that severs a route costs almost no IoU
but matters greatly for routing applications. Third, the OSM completeness
measure is bounded by systematic differences between OSM's definition of a
road and what is visible in 1\,m aerial imagery, which is why the human
ground truth serves as the appropriate reference point.
...
\bibitem{demir2018deepglobe} I. Demir \emph{et al.}, ``DeepGlobe 2018: A
challenge to parse the Earth through satellite images,'' in \emph{Proc. IEEE
CVPR Workshops}, 2018, pp. 172--181.
\end{thebibliography}
